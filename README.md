...
NaviPay is a local-only commerce sandbox. A user enters one purchase instruction such as `Find an Apple Magic Keyboard` and presses **Run purchase**. The default path is the seeded local merchant catalog and local gateway. The frontend keeps the primary path to one instruction, a three-stage tracker (Find item, Payment, Order), a server-owned outcome, and a receipt. Optional approved target-site evidence, payment details, and purchase evidence stay collapsed in secondary views; operator funding controls remain outside the customer surface. The server interprets the intent, returns normalized candidates and match evidence, then uses the authoritative local quote, inventory, XSGD wallet, order, fulfillment, delivery, receipt, and audit flow.
