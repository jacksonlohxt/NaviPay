...
- The canonical browser shell is the dark three-stage purchase flow in `public/app.js` and `public/styles.css`; `npm run test:ui` exercises its desktop and narrow idle, running, success, no-match, drawer, and disclosure paths through `chrome-devtools-axi`.
