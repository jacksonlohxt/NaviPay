...
- Task projections use task-owned financial snapshots and lifecycle statuses (`not_started`, `skipped`, or explicit outcomes), never the current global wallet as a fallback. Quote IDs, cart IDs, line snapshots, expiry, budget status, and stable snapshot hashes are persisted with inventory reservations; `test/truthfulness.test.js` covers no-match isolation, hard constraints, checkpoint recovery, and commit compensation. `projectCustomerOutcome` and `projectNextActions` in `src/sandbox.js` own the customer status copy, side-effects summary, and bounded supported actions; `test/customer-outcome.test.js` covers their state matrix.
- The primary UI path is seeded local commerce and the mock KYC/funding disclosure is collapsed; optional browser evidence is explicitly read-only and collapsed. The receipt is the primary success artifact and contains immutable capture snapshots plus a safe current refund/reversal adjustment, alongside price, balance, payment, order, fulfillment, delivery, and reference snapshots. See `docs/architecture.md` for the technical flow and run `npm run test:ui` for chrome-devtools-axi assertions across idle, running, success, drawer, narrow, no-match, and unknown-payment states.
- P0 agent contracts and safe projections live in `src/agent-contract.js`, `src/model-gateway.js`, and `src/agentic.js`; the default mode is the offline signed fixture replay in `fixtures/agent-replay-v1.json`, with `agentMode: deterministic_fallback` as the network-free fallback. Read-only reviewer projections are served at `/api/tasks/:id/reviewer` and `/api/runs/:runId/reviewer`; normalized agent events and checkpoints are persisted in the existing version 2 JSON store and rebuild through `NaviPaySandboxService.rebuildProjections`.
